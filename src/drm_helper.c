    ...
    for (int i = 0; i < res->count_connectors; i++) {
        ...
        if (ret) {
            if (ret != -ENOENT) {
                errno = -ret;
                fprintf(stderr,
                        "cannot setup device for connector %u, %u (%d): %m", i,
                        res->connectors[i], errno);
            }
            free(dev);
        ...
        // success
        // free the connector data we retrieved from the kernel as we copied it
        // now to our userspace. Link the drm device to our list
        drmModeFreeConnector(conn);
...
static int drm_setup_dev(struct drm_manager *drm, struct drm_dev *dev,
    ...
    // connector to our personal structure and link it with a valid CRTC. We
    // also setup a framebuffer for the image data

    // TODO: Only use the first mode for now, which should be the
    // native/recommended mode. But in reality, we could fetch all modes and
    // make some way to select one.
